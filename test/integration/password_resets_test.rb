...
class PasswordUpdateTest < PasswordResetForm
  ...
  test "update with invalid password and confirmation" do
    patch password_reset_path(@reset_user.reset_token),
          params: { email: @reset_user.email,
                    user: { password:              "foobaz1234567",
                            password_confirmation: "barquux1234567" } }
  ...
  test "update with valid password and confirmation" do
    patch password_reset_path(@reset_user.reset_token),
          params: { email: @reset_user.email,
                    user: { password:              "foobaz1234567",
                            password_confirmation: "foobaz1234567" } }
    assert is_logged_in?
    # パスワードの再設定に成功したらダイジェストをnilになるように変更すること
...
class ExpiredTokenTest < ExpiredToken
  test "should redirect to the password-reset page" do    assert_not flash.empty?
